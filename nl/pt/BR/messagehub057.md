...
configurações Java configuram a JVM TTL para que ela nunca atualize o endereço IP de um nome do host até que a JVM seja reiniciada. 
Uma configuração de exemplo é aquela que tem um gerenciador de segurança.
...
Streams, consulte [APIs para administração de tópico](/docs/services/MessageHub/messagehub104.html).
...
Não é possível usar Apache Kafka 0.8.2 simples ou a API de consumidor de alto nível com o {{site.data.keyword.messagehub}}. 
Em vez disso, é possível usar a API de consumidor do Kafka mais antiga suportada, que é a 0.9.
