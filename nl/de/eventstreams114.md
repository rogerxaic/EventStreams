...
Ausgelöst durch `Producer.send(...), Consumer.listTopics()`, wenn die Metadaten nicht abgerufen werden können. Die Ausnahmebedingung wird auch im gesendeten Callback (oder im zurückgegebenen Future) angezeigt, wenn die angeforderte Bestätigung nicht innerhalb von `request.timeout.ms` zurückgegeben wird. Der Client kann die Operation wiederholen, aber die Auswirkung der wiederholten Operation hängt von der spezifischen Operation ab. Wenn zum Beispiel das Senden einer Nachricht wiederholt wird, wird die Nachricht möglicherweise dupliziert.
