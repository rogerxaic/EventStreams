...
{{site.data.keyword.messagehub}}에 연결하는 방법은 표준 플랜을 사용하는지 아니면 엔터프라이즈 플랜을 사용하는지와 Cloud Foundry 애플리케이션에서 연결하는지 아니면 기타 외부 클라이언트에서 연결하는지에 따라 달라집니다. {{site.data.keyword.messagehub}}의 API에 연결하려면 다음과 같은 두 가지 정보를 수집해야 합니다.
...
### IBM Cloud 콘솔을 사용하여 신임 정보 및 연결 가져오기
...
### IBM Cloud CLI를 사용하여 신임 정보 가져오기 
...
<li>신임 정보를 애플리케이션에 전달하십시오. 변경사항을 적용하기 위해 애플리케이션을 다시 스테이징해야 할 수 있습니다.</li>
</ol>

## 표준 플랜의 외부 애플리케이션
{: #connect_standard_external}

Cloud Foundry 외부에서 실행 중인 애플리케이션의 경우 신임 정보는 서비스 키를 작성하여 작성됩니다. 서비스 키를 가져온 경우 선택된 방법을 사용하여 키의 세부사항을 애플리케이션에 수동으로 전달하십시오. 

### IBM Cloud 콘솔을 사용하여 신임 정보 가져오기
{: #connect_standard_external_console}

1. 원하는 Cloud Foundry 조직 및 영역에 있는지 확인하십시오.
2. 대시보드에서 Cloud Foundry {{site.data.keyword.messagehub}} 서비스를 찾으십시오.
3. 서비스 타일을 클릭하십시오.
4. **서비스 신임 정보**를 클릭하십시오.
5. **새 신임 정보**를 클릭하십시오.
6. 이름과 같은 새 신임 정보에 대한 세부사항을 입력하고 **추가**를 클릭하십시오. 새 신임 정보가 신임 정보 목록에 표시됩니다.
7. JSON 형식으로 세부사항을 표시하려면 **신임 정보 보기**를 사용하여 이 신임 정보를 클릭하십시오.
8. 신임 정보를 애플리케이션에 전달하십시오. 
...
[{{site.data.keyword.messagehub}} 리소스 보안](/docs/services/MessageHub/messagehub124.html)을 참조하십시오.
...
4. **연결**을 클릭하십시오.
5. **연결 작성**을 클릭하십시오.
6. 바인드할 {{site.data.keyword.messagehub}} 서비스 타일을 선택하고 **연결**을 클릭하십시오. 이를 통해 {{site.data.keyword.messagehub}} 서비스의 Cloud Foundry 서비스 별명이 자동으로 작성된 후 애플리케이션이 이 별명에 바인드됩니다. 변경사항을 적용하기 위해 애플리케이션을 다시 스테이징해야 할 수 있습니다.
8. 왼쪽의 **런타임** 탭을 클릭하고 가운데에 있는 **환경 변수** 탭을 선택하십시오. 이제 VCAP_SERVICES 정보를 확인할 수 있습니다. 이제 애플리케이션에서 환경 변수로 이러한 정보에 액세스할 수 있습니다. 
...
<br/>
앱이 아직 {{site.data.keyword.messagehub}}에 바인드되지 않았으므로 앱은 연결을 설정할 수 없습니다. 그러므로 <code>--no-start</code> 매개변수를 통해 애플리케이션을 푸시하여 불필요한 연결 실패를 방지하는 것이 좋습니다.</li>
<li>서비스 찾기:</br>
<code>ibmcloud resource service-instances</code></li>
<li>Cloud Foundry 서비스 별명을 작성하십시오.<br/>
...
<li>신임 정보를 애플리케이션에 전달하십시오. <br/> 변경사항을 적용하기 위해 애플리케이션을 다시 스테이징해야 할 수 있습니다.</li>
</ol>


## 엔터프라이즈 플랜의 외부 애플리케이션
...
2. 서비스 타일을 클릭하십시오.
3. **서비스 신임 정보**를 클릭하십시오.
4. **새 신임 정보**를 클릭하십시오. 
5. 이름 및 역할과 같은 새 신임 정보에 대한 세부사항을 완료한 후 **추가**를 클릭하십시오. 새 신임 정보가 신임 정보 목록에 표시됩니다.
6. JSON 형식으로 세부사항을 표시하려면 **신임 정보 보기**를 사용하여 이 신임 정보를 클릭하십시오.
7. 신임 정보를 애플리케이션에 전달하십시오. 애플리케이션이 세부사항을 구문 분석하는지 확인하십시오.
...
<li>신임 정보를 애플리케이션에 전달하십시오. 애플리케이션이 세부사항을 구문 분석하는지 확인하십시오.</li>
</ol>

## 다음에 수행할 작업
{: #after_connecting}

이제 연결되어 신임 정보를 얻게 됨에 따라 {{site.data.keyword.messagehub}} 클라이언트를 선택할 수 있습니다. 사용자의 플랜에 따라 선택사항이 달라집니다. 

* 표준 플랜을 사용 중인 경우 선택할 클라이언트와 연결 방법에 대한 정보를 보려면
[세 개의 API 중에서 선택](/docs/services/MessageHub/messagehub087.html)을 참조하십시오.
* 엔터프라이즈 플랜을 사용 중인 경우 [Kafka API 사용](/docs/services/MessageHub/messagehub050.html)을 참조하십시오.
