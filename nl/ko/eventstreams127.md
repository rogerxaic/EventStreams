...
{{site.data.keyword.messagehub}}에 연결하는 방법은 사용하는 플랜(표준 또는 엔터프라이즈) 및 연결을 시작하는 위치(Cloud Foundry 애플리케이션 또는 기타 외부 클라이언트)에 따라 달라집니다. {{site.data.keyword.messagehub}}의 API에 연결하려면 다음과 같은 두 가지 정보를 수집해야 합니다.
{: shortdesc}
